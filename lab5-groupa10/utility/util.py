...
def load_toy():
    ...
    y = np.where(data.target[idx] > 0, 0, 1) # make binary, convert to 0,1
...
def logistic_gradient(X, y, theta):
    '''Computes the gradient of the logistic loss function with respect to theta, with classes 0 and 1'''

    N, _ = X.shape

    return -2/N * X.T.dot(sigmoid(X.dot(theta)) - y)
